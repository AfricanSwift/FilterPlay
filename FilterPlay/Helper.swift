...
public struct Helper
{
  ...
  public static func initializeExampleFilters(
    image: NSImage,
    stackView: NSStackView,
    scrollView: NSScrollView,
    imageRatio: CGFloat = 2.0)
  {
    
    // Clear out any prior subviews
    stackView.subviews.forEach {
      $0.removeFromSuperview()
    }
    
    // Clear out any prior stackView constraints
    stackView.constraints.forEach {
      stackView.removeConstraint($0)
    }
    
    // Clear out any prior scrollView constraints
    ...
    // Resize window to match image width
    ...
    NSLayoutConstraint(
      ...
      multiplier: 1,
      constant: subWidth).isActive = true
    ...
    NSLayoutConstraint(
      ...
      attribute: .notAnAttribute,
      multiplier: 1,
      constant: subHeight * CGFloat(images.count)).isActive = true
  ...
  private static func buildImageViews(
    images: [NSImage],
    imageRatio: CGFloat) -> [NSImageView]
  {
    let subWidth = images[0].size.width / imageRatio
    let subHeight = images[0].size.height / imageRatio
