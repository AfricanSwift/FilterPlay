...
public extension NSImage {
  public func filterBinary(level: Double = 1.0 / 2.0, threshold: Double = 0.0, transparent: Bool = false) -> NSImage? {
    ...
    let filter = Transform.Binary(level: level, threshold: threshold, transparent: transparent)
...
extension NSImage {
  ...
  internal struct Transform {
    // Color Black & White
    static func Binary(level: Double, threshold: Double, transparent: Bool) -> FilterTransform {
      return { (components: Components) -> Pixel in
        guard components.red >= threshold && components.green >= threshold && components.blue >= threshold else { return components.toPixel() }
...
public extension NSImage {
  ...
  public func filterGray(tone: GrayTone = .median, threshold: Double = 0) -> NSImage? {
    ...
    let filter = Transform.gray(tone: tone, threshold: threshold)
    return processFilterTransform(filter: filter)
  }
}
...
{
  // Gray Tones
  static func gray(tone: NSImage.GrayTone, threshold: Double) -> NSImage.FilterTransform {
...
private extension NSImage.Transform {
  ...
  static func tint(red: Double, green: Double, blue: Double, threshold: Double) -> NSImage.FilterTransform {
    ...
    return { (components: NSImage.Components) -> NSImage.Pixel in
      guard components.red >= threshold && components.green >= threshold && components.blue >= threshold else {
