  ...
  internal struct Components
  {
    ...
    func toPixel() -> NSImage.Pixel
    {
      return NSImage.Pixel(
        red: Components.toUInt8(value: self.red),
        green: Components.toUInt8(value: self.green),
        blue: Components.toUInt8(value: self.blue),
        alpha: Components.toUInt8(value: self.alpha))
    }
...
public extension NSImage
{
  ...
  private func processFilterTransform(
    filter: FilterTransform) -> NSImage?
  {
...
public extension NSImage
{
  public func filterBinary(
    level: Double = 1.0 / 2.0,
    threshold: Double = 0.0,
    transparent: Bool = false) -> NSImage?
  {
    precondition(level.isWithinLimits() && threshold.isWithinLimits(),
                 "Input values out of scope -1.0 to 1.0")
    
    let filter = Transform.Binary(
      level: level,
      threshold: threshold,
...
private extension NSImage
{
  ...
  private struct Transform
  {
    ...
    static func Binary(
      level: Double,
      threshold: Double,
      transparent: Bool) -> FilterTransform
    {
      return { (components: Components) -> Pixel in
        ...
        let alpha = components.median() > level ?
          transparent ? 0 : components.alpha : components.alpha
        return Pixel(
          red: color,
          green: color,
          blue: color,
...
public extension NSImage
{
  ...
  public func filterGray(
    tone: GrayTone = .median,
    threshold: Double = 0) -> NSImage?
  {
    precondition(threshold.isWithinLimits(),
                 "Input values out of scope -1.0 to 1.0")
    
    let filter = Transform.gray(
      tone: tone,
      threshold: threshold)
    return processFilterTransform(filter: filter)
  }
...
private extension NSImage.Transform
{
  // Gray Tones
  static func gray(
    tone: NSImage.GrayTone,
    threshold: Double) -> NSImage.FilterTransform
  {
    return { (components: NSImage.Components) -> NSImage.Pixel in
      ...
      switch tone {
      case .bright:
        colorTone = components.red
      case .median:
        colorTone = components.green
      case .dark:
        colorTone = components.blue
      case .luminosity:
        colorTone = components.median()
      }
...
private extension NSImage.Transform
{
    ...
    threshold: Double) -> NSImage.FilterTransform
  {
    ...
    return { (components: NSImage.Components) -> NSImage.Pixel in
      guard components.median() > threshold else
      {
        return components.toPixel()
      }
      
      return NSImage.Pixel(
        red: red == initial ? components.red :
          components.red < red ? 1.0 - components.red : components.red,
        green: green == initial ? components.green :
          components.green < green ? 1.0 - components.green : components.green,
        blue: blue == initial ? components.blue :
          components.blue < blue ? 1.0 - components.blue :  components.blue,
        alpha: components.alpha)
    }
  }
}
...
public extension NSImage
{
  public func filterInvert(
    threshold: Double = 0.0) -> NSImage?
  {
    precondition(threshold.isWithinLimits(),
                 "Input values out of scope -1.0 to 1.0")
...
public extension NSImage
{
    ...
    threshold: Double = 0.0) -> NSImage?
  {
    ...
    let filter = Transform.brightness(
      level: level,
      threshold: threshold)
    return processFilterTransform(filter: filter)
  }
